...
pinetime_directory = config['paths_pinetime']['pinetime_directory']
...
for (dirpath, dirnames, filenames) in walk(pinetime_directory):
...
def compute_trial_file(trial_file_name, window_size):
    ...
    df_Mediciones = pd.DataFrame(pd.read_csv(pinetime_directory + trial_file_name, header = None, sep = ',', 
											 names = ["X_nog", "Y_nog", "Z_nog","other"], 
													 skiprows= 0))
    # We just need the triaxial information
    df_Mediciones = df_Mediciones.drop(["other"],axis = 1)    
    ...
    # Transform from bits o angular velocity
    ...
    df_Features.to_csv(unified_directory + 'Unified_PineTime_' + str(ws) + 's.txt', mode='a', header=False)
...
for window_size in [1.5,2,3]:  
    ...
    try:
        ...
        os.remove(unified_directory + 'Unified_PineTime_' + str(window_size) + 's.txt')
    except:
