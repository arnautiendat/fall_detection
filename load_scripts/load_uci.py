...
# %% Uci processed files import

# Preliminary step 0. We need to establish/select our working folders. 
uci_directory = config['paths_uci']['uci_directory']
...
for (dirpath, dirnames, filenames) in walk(uci_directory):
    file_names.extend(filenames)
    for filename in filenames:
        if(filename =='340537.txt'):
...
df_Files_Trials = df_Files_Trials[df_Files_Trials.File =='340537.txt'].reset_index()

df_Files_Trials['Dirs'] = dir_names
...
df_Mediciones_all = pd.read_csv(dir_name, header = 0, sep = '\t', 
                                names = ['Counter','Temperature',
                                         'VelInc_X','VelInc_Y','VelInc_Z',
                                         'OrInc_w','OriInc_x','OriInc_y','OriInc_z',
                                         'X','Y','Z',
                                         'Gyr_X','Gyr_Y','Gyr_Z','Mag_X','Mag_Y','Mag_Z',
                                         'Preassure','Roll','Pitch','Yaw','RSSI','kk'],skiprows = 7)


df_Mediciones_aux = df_Mediciones_all[["X","Y","Z"]]
fn = lambda row: row/9.80665
col = df_Mediciones_aux.apply(fn, axis=1) 
col = col.rename(columns = {"X": "S1_X","Y": "S1_Y","Z": "S1_Z"})
df_Mediciones_aux = pd.concat([df_Mediciones_aux,col],axis = 1)
...
def compute_trial_file(trial_file_name, window_size):
    ...
    df_Mediciones_all = pd.read_csv(trial_file_name, header = 0, sep = '\t', 
                                    names = ['Counter','Temperature',
                                             'VelInc_X','VelInc_Y','VelInc_Z',
                                             'OrInc_w','OriInc_x','OriInc_y','OriInc_z',
                                             'X_g','Y_g','Z_g',
    ...
    df_Mediciones_aux = df_Mediciones_all[["X_g","Y_g","Z_g"]]
    
    # Divide by gravity value
    #fn = lambda row: row/9.80665
    #col = df_Mediciones_aux.apply(fn, axis=1) 
    df_Mediciones_aux = df_Mediciones_aux.assign(X = df_Mediciones_aux['X_g'] / 9.80665)
    df_Mediciones_aux = df_Mediciones_aux.assign(Y = df_Mediciones_aux['Y_g'] / 9.80665)
    df_Mediciones_aux = df_Mediciones_aux.assign(Z = df_Mediciones_aux['Z_g'] / 9.80665)


    sf = 25 # sampling frequency
    ...
    if(int(re.findall('\d+',my_data_file_name)[1])<900):
        act = 'D'
    else:
        act = 'F'
    ...
    df_Features.to_csv(unified_directory + 'Unified_Uci_' + str(ws) + 's.txt', mode='a', header=False)
...
for window_size in [1.5,2,3]:  
    ...
    try:
        ...
        os.remove(unified_directory + 'Unified_Uci_' + str(window_size) + 's.txt')
        print("HE ENTRAT")
    except:
    ...
    for index, row in file_list.iterrows():
        ...
        print("PROCESSING TRIAL FILE: " + row['Dirs'])
