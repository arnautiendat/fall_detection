...
uma_directory = config['paths_uma']['uma_directory']
...
for (dirpath, dirnames, filenames) in walk(uma_directory):
...
def compute_trial_file(trial_file_name, window_size):
    ...
    df_Mediciones_all = pd.read_csv(uma_directory + trial_file_name, header = 0, sep = ';', 
                                         names = ['TimeStamp', 'SampleNo', 'X','Y','Z', 'Type', 'sensor_ID'], 
                                                   skiprows= 41)

    # We just need the triaxial data of one sensor
    df_Mediciones_file = df_Mediciones_all.loc[(df_Mediciones_all['Type']==0) & (df_Mediciones_all['sensor_ID']== 3)]

    df_Mediciones_aux = df_Mediciones_file[["X","Y","Z"]]

    sf = 19 # sampling frequency
    ...
    df_Features.to_csv(unified_directory + 'Unified_UMA_' + str(ws) + 's.txt', mode='a', header=False)
...
for window_size in [1.5,2,3]:  
    ...
    try:
        ...
        os.remove(unified_directory + 'Unified_UMA_' + str(window_size) + 's.txt')
    except:
